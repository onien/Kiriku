...
public class GrantsCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (!sender.hasPermission("kiriku.grants")) {
        ...
        if (args.length == 3) {
            if (args[0].equalsIgnoreCase("remove")) {
                ...
                try (Connection con = Main.getInstance().getHikari().getConnection();
                     PreparedStatement statement = con.prepareStatement("DELETE FROM grant_history WHERE ID=?")) {
        ...
        sender.sendMessage(Messages.CC("&cUsage: /grants <player>"));
        return true;
    }

    public void openMenu(OfflinePlayer player, Player playes) {
        try (Connection con = Main.getInstance().getHikari().getConnection();
             PreparedStatement statement = con.prepareStatement("SELECT * FROM grant_history WHERE UUID=?")) {
            ...
            while (result.next()) {
                ...
                Rank rk = Main.getRankManager().getByName(rank);
                if (rk == null) continue;

                int id = result.getInt("ID");

                for (String m : Messages.GRANTS_LORE) {
                    String staff = result.getString("staff");
                    String reason = result.getString("reason");
                    Long expire = result.getLong("time");
                    Boolean isTemp = result.getBoolean("Temp");
                    m = m.replace("$rank", rk.getMaincolor() + rk.getName());
                ...
                Rank r = Main.getRankManager().getByName(rank);

                meta.setDisplayName(Messages.CC(r.getMaincolor() + r.getName()));
                meta.setLore(resu);
                itemStack.setItemMeta(meta);

                pagedPane.addButton(new BadButton(
                        itemStack,
                        event -> playes.performCommand("grants remove " + id + " " + player.getName())));
            }
